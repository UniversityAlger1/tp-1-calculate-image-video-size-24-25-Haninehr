...
float video(int w, int h, int durationMovie, int durationCredits, int fps, char* unit) {
  ...
  int colorPixelSize = 24;   // 3 bytes per pixel for RGB (colored)
    ...
    totalSizeBit += totalFramesMovie * w * h * colorPixelSize;//15099x10^10
    ...
    if (strcmp(unit, "bt") == 0) {
        ...
        return totalSizeBit / (8*1024); // Convert to kilobits
    }
    else if (strcmp(unit, "mo") == 0) {
        return totalSizeBit / (8*1024*1024); // Convert to megabits
    }
    else if (strcmp(unit, "go") == 0) {
        return totalSizeBit / (8589934592); // Convert to gigabits
    }
